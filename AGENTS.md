...
Debian-only (no `-alpine` tag); their bare `:image` resolves to Debian.
...
Each image directory contains `Dockerfile.debian` (Debian variant) and, for
images with an Alpine build, `Dockerfile` (Alpine). `python`, `prose`,
`polyglot`, `jvm`, `android`, and `android-ndk` are Debian-only.

**Build system:** `docker-bake.hcl` with registry-based cache (GHCR).
Architectures: `linux/amd64` + `linux/arm64` (QEMU for cross-builds).

**Tag format:** each image publishes `:{image}-alpine` / `:{image}-debian`
(plus versioned forms) and a bare `:{image}` pointing at the recommended
variant. Tags are generated by the `img_tags()` function in `docker-bake.hcl`
(`bare = true` claims the bare tag). See README → "Choosing a variant".
